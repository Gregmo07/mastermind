...
colorChoice.forEach((element) => {
  element.addEventListener("click", (e) => {
    if (getChoosedColorDot())
      getChoosedColorDot().classList.remove("selectedColor");
  ...
  for (let i = 0; i < guessedColorArray.length; i++) {
    if (guessedColorArray[i] === puzzle[i]) {
      ...
      puzzle.forEach((element, index) => {
        ...
        if (
          element == guessedColorArray[i] &&
          element != guessedColorArray[index] &&
          !savedArray.includes(index) &&
          !exit
        ) {
...
function checkResult() {
  ...
  array.forEach((element) => {
    ...
    document.querySelector(
      ".result>.activeLine>.dot:nth-child(" + i + ")"
    ).style.backgroundColor = element;
  });
  document.querySelector(".result>.activeLine").style   .visibility = "visible";
  changeActiveLine();
}
